import { useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import { apiFetch } from '../api';
import { useAuth } from '../AuthContext';
import AssignControls from './AssignControls';
import Icon from './Icon';
import StatusPill from './StatusPill';
import StatusTimeline from './StatusTimeline';

export const shortId = issue =>
  `LS-${new Date(issue.createdAt).getFullYear()}-${String(issue._id).slice(-6).toUpperCase()}`;

const fullDate = iso => new Date(iso).toLocaleString(undefined, {
  day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit',
});

export default function IssueDrawer({ issue, onClose, onSaved, onUpdateStatus }) {
  const { user } = useAuth();
  const isAdmin = user?.role === 'admin';
  const [actionLoading, setActionLoading] = useState(false);
  const [actionError, setActionError] = useState(null);

  useEffect(() => {
    const onKey = e => e.key === 'Escape' && onClose();
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [onClose]);

  const evidence = [
    ...(issue.photos || []).map(src => ({ src, label: 'Citizen Reported Photo' })),
    ...(issue.resolution?.evidence || []).map(src => ({ src, label: 'Resolution Proof Photo' })),
    ...(issue.statusHistory || [])
      .filter(h => h.evidence && !(issue.resolution?.evidence || []).includes(h.evidence))
      .map(h => ({ src: h.evidence, label: `Evidence — ${h.status}` })),
  ];

  // Quick Admin Verification
  async function handleAdminVerify(action) {
    setActionLoading(true);
    setActionError(null);
    try {
      const updated = await apiFetch(`/api/issues/${issue._id}/verify-resolution`, {
        method: 'POST',
        body: JSON.stringify({
          action,
          adminNotes: action === 'approve' ? 'Resolution proof verified by administrator' : 'Resolution rejected by admin. Rework required.',
        }),
      });
      onSaved?.(updated);
    } catch (e) {
      setActionError(e.message);
    } finally {
      setActionLoading(false);
    }
  }

  // Quick Admin Reopen
  async function handleAdminReopen() {
    setActionLoading(true);
    setActionError(null);
    try {
      const updated = await apiFetch(`/api/issues/${issue._id}/reopen`, {
        method: 'POST',
        body: JSON.stringify({ note: 'Reopened by Admin following citizen dissatisfaction.' }),
      });
      onSaved?.(updated);
    } catch (e) {
      setActionError(e.message);
    } finally {
      setActionLoading(false);
    }
  }

  return (
    <aside
      aria-label="Issue detail"
      className="fixed inset-y-0 right-0 z-40 flex w-full max-w-md flex-col border-l border-line bg-surface shadow-xl lg:top-16"
    >
      <header className="flex items-start gap-2 border-b border-line p-5">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <Link
              to={`/issues/${issue._id}`}
              target="_blank"
              className="inline-flex items-center gap-1 text-sm font-medium text-brand-600 hover:underline"
            >
              #{shortId(issue)}
              <Icon name="external" className="size-3.5" />
            </Link>
            <StatusPill status={issue.status} size="sm" />
          </div>
          <h2 className="mt-1.5 text-base font-semibold leading-snug text-ink">{issue.title}</h2>
          <p className="mt-1 flex items-center gap-1.5 text-xs text-ink-muted">
            <Icon name="map" className="size-3.5 shrink-0 text-slate-400" />
            {issue.address || issue.area || 'Location not given'}
          </p>
          <p className="mt-0.5 text-[11px] text-ink-muted">Submitted {fullDate(issue.createdAt)}</p>

          {/* Assigned Officer Pill */}
          {issue.assignedOfficer && (
            <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-0.5 text-xs text-slate-700 border border-slate-200">
              <Icon name="shield" className="size-3 text-brand-600" />
              <span>Assigned Officer: <strong>{issue.assignedOfficer.name || 'Officer'}</strong></span>
            </div>
          )}
        </div>

        <button
          type="button"
          onClick={onClose}
          aria-label="Close detail panel"
          className="cursor-pointer rounded-lg p-2 text-ink-muted transition-colors hover:bg-canvas hover:text-ink"
        >
          <Icon name="close" className="size-4" />
        </button>
      </header>

      <div className="flex-1 space-y-5 overflow-y-auto p-5 text-sm">
        {issue.description && (
          <p className="text-xs sm:text-sm text-ink-muted leading-relaxed">{issue.description}</p>
        )}

        {/* ── Pending Verification Banner (Admin) ── */}
        {issue.status === 'Pending Verification' && (
          <div className="rounded-xl border border-purple-200 bg-purple-50 p-3.5">
            <div className="flex items-center gap-2 text-xs font-bold text-purple-900">
              <span className="size-2 rounded-full bg-purple-600 animate-pulse" />
              Proof Awaiting Admin Verification
            </div>
            <p className="mt-1 text-xs text-purple-800">
              The officer has submitted resolution photos and notes. Review the images below to approve or reject.
            </p>
            {issue.resolution?.note && (
              <p className="mt-2 rounded-lg bg-white/80 p-2 text-xs text-ink font-medium border border-purple-100">
                "{issue.resolution.note}"
              </p>
            )}

            {isAdmin && (
              <div className="mt-3 flex gap-2">
                <button
                  type="button"
                  disabled={actionLoading}
                  onClick={() => handleAdminVerify('approve')}
                  className="flex-1 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white shadow-xs hover:bg-emerald-700 disabled:opacity-50"
                >
                  {actionLoading ? 'Saving…' : '✓ Approve & Close'}
                </button>
                <button
                  type="button"
                  disabled={actionLoading}
                  onClick={() => handleAdminVerify('reject')}
                  className="flex-1 rounded-lg border border-rose-300 bg-white px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50"
                >
                  ✕ Reject Proof
                </button>
              </div>
            )}
          </div>
        )}

        {/* ── Unsatisfied Banner (Admin) ── */}
        {issue.status === 'Unsatisfied' && (
          <div className="rounded-xl border border-rose-200 bg-rose-50 p-3.5">
            <div className="flex items-center gap-2 text-xs font-bold text-rose-900">
              <span className="size-2 rounded-full bg-rose-600" />
              Citizen Unsatisfied with Resolution
            </div>
            <p className="mt-1 text-xs text-rose-800">
              The reporter marked this issue unsatisfied:
            </p>
            {issue.citizenFeedback?.notes && (
              <p className="mt-2 rounded-lg bg-white/80 p-2 text-xs text-rose-950 font-medium border border-rose-100 italic">
                "{issue.citizenFeedback.notes}"
              </p>
            )}

            {isAdmin && (
              <button
                type="button"
                disabled={actionLoading}
                onClick={handleAdminReopen}
                className="mt-3 w-full rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white shadow-xs hover:bg-rose-700 disabled:opacity-50"
              >
                {actionLoading ? 'Reopening…' : '↺ Reopen Issue (Move to In Progress)'}
              </button>
            )}
          </div>
        )}

        {/* ── Closed Banner ── */}
        {issue.status === 'Closed' && (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-900">
            <span className="font-bold">✓ Officially Closed:</span> Citizen accepted the resolution.
            {issue.citizenFeedback?.notes && (
              <p className="mt-1 text-[11px] italic text-emerald-800">"{issue.citizenFeedback.notes}"</p>
            )}
          </div>
        )}

        {actionError && (
          <p className="text-xs text-rose-600 font-medium">{actionError}</p>
        )}

        {/* Photos & Evidence */}
        {evidence.length > 0 && (
          <section>
            <h3 className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-ink-muted">
              <Icon name="photo" className="size-3.5 text-ink-muted" />
              Photos &amp; Evidence ({evidence.length})
            </h3>
            <div className="grid grid-cols-3 gap-2">
              {evidence.map(({ src, label }, idx) => (
                <a
                  key={idx}
                  href={src}
                  target="_blank"
                  rel="noreferrer"
                  title={label}
                  className="group relative overflow-hidden rounded-lg border border-line aspect-square"
                >
                  <img src={src} alt={label} loading="lazy" className="size-full object-cover transition-transform group-hover:scale-105" />
                  <span className="absolute bottom-0 inset-x-0 bg-ink/70 p-1 text-[9px] text-white truncate text-center">
                    {label}
                  </span>
                </a>
              ))}
            </div>
          </section>
        )}

        {/* Status Timeline */}
        <section>
          <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-ink-muted">Full Audit Trail</h3>
          {issue.statusHistory?.length > 0 ? (
            <StatusTimeline history={issue.statusHistory} />
          ) : (
            <p className="text-xs text-ink-muted">No status changes yet.</p>
          )}
        </section>
      </div>

      {/* Footer Controls */}
      <footer className="space-y-3 border-t border-line bg-surface p-4">
        {isAdmin && (
          <div>
            <h3 className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-ink-muted">
              Triage &amp; Allotment
            </h3>
            <AssignControls issue={issue} onSaved={onSaved} />
          </div>
        )}

        <div className="flex gap-2 pt-1">
          <button
            type="button"
            onClick={() => onUpdateStatus(issue)}
            className="flex-1 rounded-xl bg-brand-600 py-2.5 text-xs font-semibold text-white shadow-xs hover:bg-brand-700 transition-colors cursor-pointer"
          >
            Update / Report Resolution
          </button>
        </div>
      </footer>
    </aside>
  );
}
